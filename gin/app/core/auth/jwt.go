...
// 签发token
func (j *Jwt) GenerateJwtToken(subject string, audience []string, ttl time.Duration) (string, error) {
	now := time.Now()
...
// 解析token
func (j *Jwt) ParseJwtToken(tokenString string) (string, []string, error) {
